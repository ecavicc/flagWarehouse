...
if __name__ == '__main__':
    ...
    for i in range(10000):
        status = 'NOT_SUBMITTED'  # random.choices(population=list(CONFIG.DB_STATUS.values()), weights=(0.1, 1), k=1)[0]
        response = None
        if status == 'SUBMITTED':
            response = random.choice(['SUCCESS', 'ERROR'])
        rows.append(('FLG{{{}}}'.format(str(i).zfill(10)), random.choice(usernames), random.choice(exploits),
                     random.choice(ips), rand_time(hrs=0.001).replace(microsecond=0).isoformat(sep=' '), status))
